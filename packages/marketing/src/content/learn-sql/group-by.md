...
Repeating the expression is tedious. PostgreSQL, MySQL, SQLite and DuckDB all let you group by the output alias or by column position:
...
Date truncation is the usual real-world case, and every engine spells it differently. PostgreSQL and DuckDB have `DATE_TRUNC('month', created_at)`, MySQL has `DATE_FORMAT(created_at, '%Y-%m')`, SQLite uses `strftime('%Y-%m', created_at)`.
...
There's no way to invent those rows with `GROUP BY` alone. You generate the full list of buckets separately and `LEFT JOIN` your aggregated data onto it. `generate_series` does the generating in PostgreSQL and DuckDB; elsewhere you keep a calendar table around.
...
And `GROUP BY c.id, ...` rather than grouping on names. Two customers could share a name; ids can't collide. Grouping on the primary key and listing the other columns alongside is the safe habit. PostgreSQL and DuckDB will actually let you write `GROUP BY c.id` alone and select the other columns, because they can prove the key determines them, but spelling it out works everywhere.
