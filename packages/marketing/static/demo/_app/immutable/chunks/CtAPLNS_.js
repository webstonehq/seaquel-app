const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./DKdY3bmn.js","./RciSkj6z.js","./Cyza1jiY.js","./DV-YQ2BS.js","./Ay-7V1Px.js","./PPVm8Dsz.js","./TUVgSCCE.js","./2iOLhxB2.js"])))=>i.map(i=>d[i]);
import{_ as c}from"./PPVm8Dsz.js";import{i as R}from"./DUS61wQs.js";const m=1,O=[`CREATE TABLE IF NOT EXISTS schema_version (
    ...
    last_check_timestamp TEXT
  )`];async function S(a){if((await a.query("SELECT name FROM sqlite_master WHERE type='table' AND name='schema_version'")).length>0)return!1;const i=O.map(t=>({sql:t}));return await a.transaction(i),!0}let d=null,E=null;async function I(){return d||E||(E=(async()=>{let a;if(R()){const{getDataDir:i}=await c(async()=>{const{getDataDir:r}=await import("./DKdY3bmn.js").then(T=>T.E);return{getDataDir:r}},__vite__mapDeps([0,1]),import.meta.url),{TauriSqliteProvider:t}=await c(async()=>{const{TauriSqliteProvider:r}=await import("./Cyza1jiY.js");return{TauriSqliteProvider:r}},__vite__mapDeps([2,3,1]),import.meta.url),n=new t,o=await i();a=await n.open(`${o}/seaquel.db`)}else{const{WebSqliteProvider:i}=await c(async()=>{const{WebSqliteProvider:n}=await import("./Ay-7V1Px.js");return{WebSqliteProvider:n}},__vite__mapDeps([4,5]),import.meta.url);a=await new i().open("seaquel.db")}if(await a.execute("PRAGMA journal_mode=WAL"),await a.execute("PRAGMA foreign_keys=ON"),await S(a)){const{migrateJsonToSqlite:i}=await c(async()=>{const{migrateJsonToSqlite:t}=await import("./TUVgSCCE.js");return{migrateJsonToSqlite:t}},__vite__mapDeps([6,5,7]),import.meta.url);await i(a),await a.execute("INSERT INTO schema_version (version) VALUES (?)",[m])}else if((await a.query("SELECT COUNT(*) as count FROM connections"))[0].count===0){const{migrateJsonToSqlite:t}=await c(async()=>{const{migrateJsonToSqlite:n}=await import("./TUVgSCCE.js");return{migrateJsonToSqlite:n}},__vite__mapDeps([6,5,7]),import.meta.url);await t(a)}return d=a,a})(),E)}const h=Object.freeze(Object.defineProperty({__proto__:null,getDatabase:I},Symbol.toStringTag,{value:"Module"})),w={async loadAll(a){const e=await a.query("SELECT id, name, description, created_at, updated_at FROM projects"),i=[];for(const t of e){const n=await a.query("SELECT id, name, is_predefined, color FROM project_labels WHERE project_id = ?",[t.id]);i.push({id:t.id,name:t.name,description:t.description??void 0,createdAt:t.created_at,updatedAt:t.updated_at,customLabels:n.map(o=>({id:o.id,name:o.name,isPredefined:o.is_predefined===1,color:o.color}))})}return i},async save(a,e){await a.execute(`INSERT OR REPLACE INTO projects (id, name, description, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?)`,[e.id,e.name,e.description??null,e.createdAt,e.updatedAt]),await a.execute("DELETE FROM project_labels WHERE project_id = ?",[e.id]);for(const i of e.customLabels)await a.execute(`INSERT INTO project_labels (id, project_id, name, is_predefined, color)
         VALUES (?, ?, ?, ?, ?)`,[i.id,e.id,i.name,i.isPredefined?1:0,i.color])},async saveAll(a,e){for(const i of e)await this.save(a,i)},async remove(a,e){await a.execute("DELETE FROM projects WHERE id = ?",[e])}},_={async get(a,e){const i=await a.query("SELECT value FROM app_state WHERE key = ?",[e]);return i.length===0?null:i[0].value},async set(a,e,i){await a.execute("INSERT OR REPLACE INTO app_state (key, value) VALUES (?, ?)",[e,i])}},b={async loadAll(a){const e=await a.query("SELECT * FROM connections"),i=[];for(const t of e){const n=await a.query("SELECT label_id FROM connection_labels WHERE connection_id = ?",[t.id]);i.push({id:t.id,projectId:t.project_id,name:t.name,type:t.type,host:t.host,port:t.port,databaseName:t.database_name,username:t.username,sslMode:t.ssl_mode??void 0,connectionString:t.connection_string??void 0,lastConnected:t.last_connected?new Date(t.last_connected):void 0,sshTunnel:t.ssh_tunnel?JSON.parse(t.ssh_tunnel):void 0,savePassword:t.save_password===1,saveSshPassword:t.save_ssh_password===1,saveSshKeyPassphrase:t.save_ssh_key_passphrase===1,labelIds:n.map(o=>o.label_id)})}return i},async save(a,e){await a.execute(`INSERT OR REPLACE INTO connections
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[e.id,e.projectId,e.name,e.type,e.host,e.port,e.databaseName,e.username,e.sslMode??null,e.connectionString??null,e.lastConnected instanceof Date?e.lastConnected.toISOString():e.lastConnected??null,e.sshTunnel?JSON.stringify(e.sshTunnel):null,e.savePassword?1:0,e.saveSshPassword?1:0,e.saveSshKeyPassphrase?1:0]),await a.execute("DELETE FROM connection_labels WHERE connection_id = ?",[e.id]);for(const i of e.labelIds)await a.execute("INSERT INTO connection_labels (connection_id, label_id) VALUES (?, ?)",[e.id,i])},async remove(a,e){await a.execute("DELETE FROM connections WHERE id = ?",[e])}},X={async load(a,e){const i=await a.query("SELECT * FROM project_state WHERE project_id = ?",[e]);if(i.length===0)return null;const t=i[0],n=await a.query("SELECT * FROM tabs WHERE project_id = ?",[e]),o=n.filter(s=>s.tab_type==="query").map(s=>({id:s.id,name:s.name,query:s.query??"",savedQueryId:s.saved_query_id??void 0,sharedQueryId:s.shared_query_id??void 0})),r=n.filter(s=>s.tab_type==="schema").map(s=>({id:s.id,tableName:s.table_name??"",schemaName:s.schema_name??""})),T=n.filter(s=>s.tab_type==="explain").map(s=>({id:s.id,name:s.name,sourceQuery:s.source_query??""})),l=n.filter(s=>s.tab_type==="erd").map(s=>({id:s.id,name:s.name,connectionId:s.connection_id??void 0})),p=n.filter(s=>s.tab_type==="statistics").map(s=>({id:s.id,name:s.name,connectionId:s.connection_id??""})),u=n.filter(s=>s.tab_type==="canvas").map(s=>({id:s.id,name:s.name,connectionId:s.connection_id??""})),N=n.filter(s=>s.tab_type==="starter").map(s=>({id:s.id,type:s.starter_type??"getting-started",name:s.name,closable:s.closable===1})),L=(await a.query("SELECT id, data FROM saved_canvases WHERE project_id = ?",[e])).map(s=>JSON.parse(s.data));return{projectId:e,queryTabs:o,schemaTabs:r,explainTabs:T,erdTabs:l,statisticsTabs:p,canvasTabs:u,tabOrder:JSON.parse(t.tab_order),activeQueryTabId:t.active_query_tab_id,activeSchemaTabId:t.active_schema_tab_id,activeExplainTabId:t.active_explain_tab_id,activeErdTabId:t.active_erd_tab_id,activeStatisticsTabId:t.active_statistics_tab_id,activeCanvasTabId:t.active_canvas_tab_id,activeView:t.active_view,activeConnectionId:t.active_connection_id,starterTabs:N,activeStarterTabId:t.active_starter_tab_id,savedCanvases:L}},async save(a,e){await a.execute(`INSERT OR REPLACE INTO project_state
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[e.projectId,e.activeView,e.activeConnectionId,e.activeQueryTabId,e.activeSchemaTabId,e.activeExplainTabId,e.activeErdTabId,e.activeStatisticsTabId??null,e.activeCanvasTabId??null,null,e.activeStarterTabId??null,JSON.stringify(e.tabOrder)]),await a.execute("DELETE FROM tabs WHERE project_id = ?",[e.projectId]);for(const i of e.queryTabs)await a.execute(`INSERT INTO tabs (id, project_id, tab_type, name, query, saved_query_id, shared_query_id)
         ...
         VALUES (?, ?, 'starter', ?, ?, ?)`,[i.id,e.projectId,i.name,i.type,i.closable?1:0]);await a.execute("DELETE FROM saved_canvases WHERE project_id = ?",[e.projectId]);for(const i of e.savedCanvases??[])await a.execute("INSERT INTO saved_canvases (id, project_id, data) VALUES (?, ?, ?)",[i.id??crypto.randomUUID(),e.projectId,JSON.stringify(i)])},async remove(a,e){await a.execute("DELETE FROM project_state WHERE project_id = ?",[e]),await a.execute("DELETE FROM tabs WHERE project_id = ?",[e]),await a.execute("DELETE FROM saved_canvases WHERE project_id = ?",[e])}},C={async loadByConnection(a,e){return(await a.query("SELECT * FROM saved_queries WHERE connection_id = ?",[e])).map(t=>({id:t.id,connectionId:t.connection_id,name:t.name,query:t.query,parameters:t.parameters?JSON.parse(t.parameters):void 0,createdAt:t.created_at,updatedAt:t.updated_at}))},async saveAll(a,e,i){await a.execute("DELETE FROM saved_queries WHERE connection_id = ?",[e]);for(const t of i)await a.execute(`INSERT INTO saved_queries (id, connection_id, name, query, parameters, created_at, updated_at)
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,[t.id,e,t.query,t.timestamp,t.executionTime,t.rowCount,t.favorite?1:0,t.connectionLabelsSnapshot?JSON.stringify(t.connectionLabelsSnapshot):null,t.connectionNameSnapshot])},async removeByConnection(a,e){await a.execute("DELETE FROM query_history WHERE connection_id = ?",[e])}},U={async loadAll(a){const i=(await a.query("SELECT id, data FROM shared_repos")).map(n=>JSON.parse(n.data)),t=await _.get(a,"activeRepoId");return{repos:i,activeRepoId:t}},async saveAll(a,e,i){await a.execute("DELETE FROM shared_repos");for(const t of e)await a.execute("INSERT INTO shared_repos (id, data) VALUES (?, ?)",[t.id,JSON.stringify(t)]);await _.set(a,"activeRepoId",i)}},F={async loadPreferences(a){const e=await a.query("SELECT light_theme_id, dark_theme_id FROM theme_preferences WHERE id = 1");return e.length===0?null:{lightThemeId:e[0].light_theme_id,darkThemeId:e[0].dark_theme_id}},async savePreferences(a,e,i){await a.execute(`INSERT OR REPLACE INTO theme_preferences (id, light_theme_id, dark_theme_id)
       VALUES (1, ?, ?)`,[e,i])},async loadUserThemes(a){return(await a.query("SELECT id, data FROM user_themes")).map(i=>JSON.parse(i.data))},async saveUserThemes(a,e){await a.execute("DELETE FROM user_themes");for(const i of e){const t=i;await a.execute("INSERT INTO user_themes (id, data) VALUES (?, ?)",[t.id,JSON.stringify(i)])}}},q={async load(a){const e=await a.query("SELECT data FROM license_state WHERE id = 1");return e.length===0?null:JSON.parse(e[0].data)},async save(a,e){await a.execute("INSERT OR REPLACE INTO license_state (id, data) VALUES (1, ?)",[JSON.stringify(e)])}},D={async load(a){const e=await a.query("SELECT data FROM onboarding_state WHERE id = 1");return e.length===0?null:JSON.parse(e[0].data)},async save(a,e){await a.execute("INSERT OR REPLACE INTO onboarding_state (id, data) VALUES (1, ?)",[JSON.stringify(e)])}},x={async loadAll(a){return a.query("SELECT lesson_id as lessonId, challenge_id as challengeId, state FROM tutorial_progress")},async save(a,e,i,t){await a.execute(`INSERT OR REPLACE INTO tutorial_progress (lesson_id, challenge_id, state)
       VALUES (?, ?, ?)`,[e,i,t])},async removeLesson(a,e){await a.execute("DELETE FROM tutorial_progress WHERE lesson_id = ?",[e])},async removeAll(a){await a.execute("DELETE FROM tutorial_progress")}},g={async load(a,e){const i=await a.query("SELECT has_offered_import, last_check_timestamp FROM import_state WHERE source = ?",[e]);return i.length===0?null:{hasOfferedImport:i[0].has_offered_import===1,lastCheckTimestamp:i[0].last_check_timestamp}},async save(a,e,i,t){await a.execute(`INSERT OR REPLACE INTO import_state (source, has_offered_import, last_check_timestamp)
       VALUES (?, ?, ?)`,[e,i?1:0,t])}};export{_ as a,X as b,b as c,U as d,x as e,h as f,I as g,g as i,q as l,D as o,w as p,f as q,C as s,F as t};
